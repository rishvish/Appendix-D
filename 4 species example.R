...
########## This code chunk from line 15 to 148 can take upto a day to run
########## If it is not desirable to run this code for that long
########## Skip to line 150 to load the pre-computed objects and verify results
################################################################################
...
set.seed(163)
coeff_interactionsADD <- round(rnorm(4, mean = 8, sd = 4), 2)

# FG interaction effects coefficients
set.seed(583)
coeff_interactionsFG <- round(c(rnorm(1, mean = 8, sd = 2), 
                                rnorm(2, mean = 6, sd = 2)), 2)

# AV interaction effects coefficients
set.seed(294)
coeff_interactionsAV <- round(rnorm(1, mean = 9, sd = 3), 2)
...
FULL_file <- simulate_response(data=props4,         #Dataset containing proportion of species
                               ...
                               fileName = '4_species_full_pairwise')        # Filename for data file 

ADD_file <- simulate_response(data=props4,         #Dataset containing proportion of species
                              theta=  c(0.05, 0.19, 0.35, 0.48, 0.63, 0.77, 0.91, 1.00, 1.17, 1.33), #List of thetas between 0 and 1.5
                              sds=c(0.8, 0.9, 1, 1.1, 1.2),     #List of standard deviations
                              seed=300,         #Specify initial seed
                              num_datasets = 200, #Specify number of different datasets to create per combination of theta and sd
                              model='ADD',       #Choose model to estimate
                              coeff=c(coeff_iden, coeff_interactionsADD), # Coefficiens for model
                              fileName = '4_species_add_species') # Filename for data file

FG_file <- simulate_response(data=props4,         #Dataset containing proportion of species
                             theta=  c(0.05, 0.19, 0.35, 0.48, 0.63, 0.77, 0.91, 1.00, 1.17, 1.33),  #List of thetas between 0 and 1.5
                             sds=c(0.8, 0.9, 1, 1.1, 1.2),     #List of standard deviations
                             seed=500,            #Specify initial seed
                             num_datasets = 200,  #Specify number of different datasets to create per combination of theta and sd
                             model='FG',          #Choose model to estimate
                             FG=c('G','G','H','H'), #Specify Functional groups. (Needed only if model is FG)
                             coeff=c(coeff_iden, coeff_interactionsFG), #Specify coefficients for the model
                             fileName = '4_species_fg_group')      #Filename for data file

AV_file <- simulate_response(data=props4,         #Dataset containing proportion of species
                             theta=  c(0.05, 0.19, 0.35, 0.48, 0.63, 0.77, 0.91, 1.00, 1.17, 1.33),    #List of thetas between 0 and 1.5
                             sds=c(0.8, 0.9, 1, 1.1, 1.2),     #List of standard deviations
                             seed=700,           #Specify initial seed
                             num_datasets = 200, #Specify number of different datasets to create per combination of theta and sd
                             model='AV',         #Choose model to estimate
                             coeff=c(coeff_iden, coeff_interactionsAV), # Specify coefficients for the model
                             fileName = '4_species_av_pairwise')      # Filename for data file
...
#write_csv(estimatesFULL, 'Estimates (4_species_FULL).csv')
#write_csv(estimatesFULL, 'Estimates (4_species_FG).csv')
#write_csv(estimatesFULL, 'Estimates (4_species_ADD).csv')
#write_csv(estimatesFULL, 'Estimates (4_species_AV).csv')
...
#write_csv(FULL_model_selection_a, 'Model Selection (4_species_FULL_a).csv')
#write_csv(AV_model_selection_a, 'Model Selection (4_species_FG_a).csv')
#write_csv(FG_model_selection_a, 'Model Selection (4_species_ADD_a).csv')
#write_csv(ADD_model_selection_a, 'Model Selection (4_species_AV_a).csv')

#write_csv(FULL_model_selection_b, 'Model Selection (4_species_FULL_b).csv')
#write_csv(AV_model_selection_b, 'Model Selection (4_species_FG_b).csv')
#write_csv(FG_model_selection_b, 'Model Selection (4_species_ADD_b).csv')
#write_csv(ADD_model_selection_b, 'Model Selection (4_species_AV_b).csv')

#write_csv(FULL_model_selection_c, 'Model Selection (4_species_FULL_c).csv')
#write_csv(AV_model_selection_c, 'Model Selection (4_species_FG_c).csv')
#write_csv(FG_model_selection_c, 'Model Selection (4_species_ADD_c).csv')
#write_csv(ADD_model_selection_c, 'Model Selection (4_species_AV_c).csv')
...
for (i in 1:length(thetas)){
  print(estimatesFULL %>% filter(sd == 1, true_theta==thetas[i]) %>% 
    ggplot(data=.)+
